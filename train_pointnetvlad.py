...
if not torch.cuda.is_available():
    raise Exception("No GPU found, please run with cuda")
...
parser.add_argument(
    "--margin_1", type=float, default=0.5, help="Margin for hinge loss [default: 0.5]"
)
parser.add_argument(
    "--margin_2", type=float, default=0.2, help="Margin for hinge loss [default: 0.2]"
)
parser.add_argument(
    "--loss_function",
    default="quadruplet",
    choices=["triplet", "quadruplet"],
    help="triplet or quadruplet [default: quadruplet]",
)
parser.add_argument(
    "--loss_not_lazy",
    action="store_false",
...
parser.add_argument(
    "--loss_ignore_zero_batch",
    action="store_true",
    help="If present, mean only batches with loss > 0.0",
)
parser.add_argument(
    "--triplet_use_best_positives",
    action="store_true",
...
parser.add_argument(
    "--dataset_folder",
    default="/lyh/benchmark_datasets/",
...
cfg.TRAIN_FILE = FLAGS.dataset_folder + "training_queries_baseline.pickle"
cfg.TEST_FILE = FLAGS.dataset_folder + "test_queries_baseline.pickle"
...
def train_one_epoch(model, optimizer, train_writer, loss_function, epoch):
    ...
    for i in tqdm(
        ...
        for j in range(cfg.BATCH_NUM_QUERIES):
            if (
                len(TRAINING_QUERIES[batch_keys[j]]["positives"])
...
def get_latent_vectors(model, dict_to_process):
    ...
    for q_index in range(
        (len(train_file_idxs) // batch_num * batch_num), len(dict_to_process.keys())
    ):
